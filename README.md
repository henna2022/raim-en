...
빈 DB로 시연을 시작하지 않도록 그럴듯한 예약 이력을 채웁니다 — 대기 3건(1건은
SLA 초과 강조), 대기자 1, 확정 1, 거절 1, 취소(yeyak 해제 대기) 1, 지난 방문일의
방문완료·노쇼, 그리고 각 단계의 발송함 메일. 데모 행은 `@demo.example` 이메일로
식별되며 **재실행하면 지우고 다시 만듭니다**(실제 신청 데이터는 건드리지 않음).
production에서는 실행이 거부되고, 실제 메일 발송과 구글 시트 미러로의 유출도
일어나지 않습니다.
...
방문객은 `/booking`에서 **예약번호만으로 상태를 조회**할 수 있습니다. 이때는 상태·날짜·회차·인원만 보이고
이름은 첫 글자만, 이메일과 거절 사유는 아예 표시되지 않습니다 — 번호를 주운 사람이 신청자를 특정하거나
연락처를 얻을 수 없어야 하기 때문입니다. **이메일까지 함께** 넣으면 전체 내용이 보이고 취소(대기자는
"Leave the waitlist")도 가능합니다. 번호는 맞고 이메일이 틀리면 가린 보기로도 흘리지 않습니다 —
그렇게 하면 이메일을 대입해 맞는 주소를 찾아내는 확인 공격이 성립하기 때문입니다.
...
신청·상태 변경이 Google 시트에 자동 반영되고, **시트의 상태 드롭다운으로 승인·거절까지
처리**할 수 있습니다. 설치는 [`scripts/apps-script/Code.gs`](scripts/apps-script/Code.gs)
상단 주석 참고 — 스크립트 속성 3개와 환경변수 3개를 맞추면 됩니다.

**① 앱 → 시트 (미러).** `SHEETS_WEBHOOK_URL` / `SHEETS_WEBHOOK_SECRET`을 비우면 꺼집니다.

**② 시트 → 앱 (상태 처리).** `SHEETS_INBOUND_SECRET`을 비우면 꺼집니다(엔드포인트가 503).
`상태` 열은 **대기(검토중) / 승인 / 거절** 드롭다운이고, 값을 바꾸면 앱의 예약 상태가 바뀌면서
신청자에게 확정·거절 메일이 나갑니다. 관리자 페이지와 **같은 전이 규칙**을 쓰므로
(`src/decisions.js`), 이미 취소된 건을 승인하는 식의 전이는 거부되고 셀이 원래 값으로 되돌아갑니다.

- **`yeyak차단` 체크박스가 켜진 행에서만 승인이 통합니다.** 관리자 페이지의 "yeyak에 좌석을
  차단했다" 확인 체크박스와 같은 안전장치입니다 — 좌석을 막지 않은 채 확정 메일이 나가면
  현장에서 방문객을 돌려보내야 합니다. 열이 없으면 승인 자체가 막힙니다(fail closed).
- **`처리자`에는 드롭다운을 바꾼 사람의 구글 계정**이 기록됩니다. 단, 구글은 시트 소유자와
  **같은 Workspace 도메인** 계정일 때만 이메일을 내줍니다. 직원이 개인 gmail로 편집하면
  `시트(편집자 미확인)`로 남습니다 — 누가 처리했는지 남겨야 한다면 기관 도메인 계정으로
  공유하세요. (하지 않은 사람에게 기록이 붙지 않도록, 소유자 계정으로 대신 채우지 않습니다.)
- 같은 값을 다시 골라도 **메일은 두 번 나가지 않습니다**(멱등).
- **한 번에 20줄을 넘겨 바꾸면 아무것도 반영하지 않습니다** — 실수로 붙여넣기 한 번에 수십 명에게
  확정 메일이 나가는 사고를 막습니다. 이때 시트에 남은 잘못된 값은 앱이 곧바로 진짜 값으로 다시 씁니다
  (`POST /api/sheet/refresh`가 그 행들을 미러 대기열에 넣습니다). 서버 응답을 받지 못한 경우에도
  같은 방식으로 맞춰지며, 그마저 실패하면 `RAIM 예약 > ④ 앱 기준으로 새로고침`으로 직접 복구합니다.
- 나머지 열은 시트에서 고쳐도 다음 동기화 때 앱의 값으로 되돌아갑니다. 원본은 앱의 DB입니다.
- 요청사항(자유입력) 원문은 시트로 내보내지 않습니다. 자유입력에 건강 관련 정보가
  섞이면 민감정보가 되어 규제 수준이 올라가기 때문입니다.
- 보존기한이 지나 앱에서 파기된 예약은 **시트에서도 자동 삭제**됩니다.
- 이전 버전이 쓰던 시트를 계속 쓴다면 새 Code.gs 배포 후 `RAIM 예약 > ④ 앱 기준으로 새로고침`을
  한 번 실행하세요 — 옛 상태 라벨('확정')로 남은 행이 현재 라벨('승인')로 다시 쓰입니다.
